...
function Unparser:unparseExpression(expression, tabbing, insideTableEntry)
	local code = "";
	...
	if(expression.kind == AstKind.StringExpression) then
		local s = "\"" .. escapeString(expression.value) .. "\"";
		if insideTableEntry then
			return "(" .. s .. ")";
		end
		return s;
	end
	...
	if(expression.kind == k) then
		...
		for i, entry in ipairs(expression.entries) do
			...
			if(entry.kind == AstKind.KeyedTableEntry) then
				...
				-- Pass insideTableEntry=true για strings
				code = code .. self:unparseExpression(entry.value, tableTabbing, true);
			end
		end
